...
const sequences: Record<string, MacroData> = {
    ...
    "RTI": {
        "label": "RTI (kernel mode)",
        "pseudocode": pseudocode`
            ${["if(PSR[15] == 1)", DISABLED]}
                ${["Initiate a privilege mode exception;", DISABLED]}
            else
                ${["PC = mem[R6]; // R6 is the SSP, PC is restored", 3]}    
                ${["R6 = R6 + 1;", 2]}     
                ${["TEMP = mem[R6]; // store in MDR", 5]}  
                R6 = R6 + 1; // pop system stack    
                PSR = TEMP; // PSR is restored
                if (PSR[15] == 1)
                     ["[Saved SSP, R6]  = [R6, Saved USP];
